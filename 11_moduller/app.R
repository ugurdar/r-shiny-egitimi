...
ui <- page_navbar(
  ...
  nav_panel(
    ...
    layout_sidebar(
      ...
      # Kartlar
      layout_columns(
        layout_columns(card(tags$img(src = "culmen_depth.png"))),
        layout_columns(
          bill_length_UI("bill_length"),
          # value_box(
...
server <- function(input, output, session) {
  ...
  bill_length_Server("bill_length", data_r)

  observe({
    showModal(modalDialog(h1("welcome")))
  })
  ...
  observe({
    updateSelectInput(
      session = session,
      inputId = "species",
      choices = c("All", as.character(unique(df$species)))
    )
  })
  ...
  observe({
    updateSelectInput(
      session = session,
      inputId = "island",
      choices = c("All", as.character(unique(df$island)))
    )
  })
  ...
  output$plot <- renderPlot({
    req(data_r())
    validate(need(nrow(data_r()) > 0, "No record found by search criterias."))
    plot_penguins(data_r(), input$variable)
  })

  output$table <- renderReactable({
    req(data_r())
    validate(need(nrow(data_r()) > 0, ""))
